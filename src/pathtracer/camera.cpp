...
Ray Camera::generate_ray(double x, double y) const {
    ...
    //
    //pos (Vector3D): camera position in world space
    //c2w (Matrix3x3): camera-to-world rotation matrix
    //initialize min_t and max_t of Ray as nclip, fclip respectively
    double hFov_rads = hFov * (double)PI / 180.0;
    double vFov_rads = vFov * (double)PI / 180.0;
    double leftx = -tan(0.5 * hFov_rads);
    double rightx = tan(0.5 * hFov_rads);
    double boty = -tan(0.5 * vFov_rads);
    double topy = tan(0.5 * vFov_rads);
    double newx = leftx + x * (rightx - leftx);
    double newy = boty + y * (topy - boty);
    Vector3D cameraspace_end = Vector3D(newx, newy, -1.0);
    Vector3D worldspace_end = c2w * cameraspace_end;
    worldspace_end.normalize();
    Ray ret(pos, worldspace_end);
    ret.min_t = nClip;
    ret.max_t = fClip;
    return ret;
